...
import { ImageIcon, VideoIcon, Sparkles, Search, ChevronDown, ChevronUp } from "lucide-react"
            ...
            <div className="grid grid-cols-2 gap-2">
              {filtered.map((template, i) => (
                <motion.button
                  ...
                  className="flex flex-col items-start gap-1.5 p-3 rounded-xl bg-zinc-900/50 border border-zinc-800/50 hover:border-purple-500/30 hover:bg-zinc-800/50 transition-all text-left group"
                >
                  ...
                  <span className="text-[10px] text-zinc-700 group-hover:text-purple-400/70 transition-colors">Click to use template</span>
                </motion.button>
              ))}
              {filtered.length === 0 && (
